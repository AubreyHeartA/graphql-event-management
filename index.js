...
const resolvers = {
    Query: {
        ...
        // Query to fetch a specific venue by ID, with error handling
        venue(_, { id }) {
            const venue = db.venues.find((venue) => venue.id === id);
            if (!venue) {
        ...
        // Query to fetch a specific event by ID, with error handling
        event(_, { id }) {
        ...
        // Query to fetch a specific ticket by ID, with error handling
        ticket(_, { id }) {
        ...
        // Query to fetch all attendees
        attendees() {
            return db.attendees;
        },
        // Query to fetch a specific attendee by ID, with error handling
        attendee(_, { id }) {
            const attendee = db.attendees.find((attendee) => attendee.id === id);
            if (!attendee) {
        ...
        // Resolver for the Event type to fetch related venues
    Event: {
        venues: (parent) => parent.venues.map((id) => db.venues.find((v) => v.id === id)),
    },

        // Resolver for the Ticket type to fetch related events
    Ticket: {
        events: (parent) => parent.events.map((id) => db.events.find((e) => e.id === id)),
    },
  
        // Resolver for the Attendee type to fetch related tickets
    Attendee: {
